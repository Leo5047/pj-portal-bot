...
def notify(cfg, openings: List[tuple]) -> None:
    if not openings:
        return
    lines = [
        f"{h} â {TERM_LABELS.get(t, t)}: {free}/{total}"
        for (h, t, free, total) in openings
    ]
    body = "\n".join(lines)
    title = f"PJ-Portal: {len(openings)} freie PlÃ¤tze ({cfg['pj_tag']})"
...
def run_once(cfg) -> None:
    ...
    log.info(f"Parsed {len(parsed)} FÃ¤cher / {total_entries} hospital entries")
